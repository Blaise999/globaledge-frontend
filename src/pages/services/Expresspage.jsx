...
export default function ExpressPage() {
  ...
  const volumetricParcel = useMemo(() => {
    const l = +parcel.length || 0, w = +parcel.width || 0, h = +parcel.height || 0;
  ...
  const volumetricFreight = useMemo(() => {
    const l = +freight.length || 0, w = +freight.width || 0, h = +freight.height || 0;
  ...
  const freightQuote = useMemo(() => {
    ...
    const eta =
      freightMode === "air" ? "2–7 days door-to-door" :
      freightMode === "sea" ? "12–35 days port-to-door" :
  ...
  function makeDraft({ serviceType, parcelLevel, freightMode, parcel, freight, from, to }) {
    if (serviceType === "freight") {
      return {
        ...
        recipientEmail: freight.recipientEmail || "",
        recipientAddress: freight.recipientAddress || "",
        // handy bundle for later use server-side
        contact: {
    ...
    <div className="min-h-screen bg-white">
          ...
          <div className="grid lg:grid-cols-2 gap-10 items-center">
            <div>
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                ...
                <Link to="/#support" className="px-4 py-2 rounded-xl ring-1 ring-inset ring-white/60 font-semibold active:scale-[0.98]">
                  Talk to support
                </Link>
              </div>
            ...
            <div id="quote" className="rounded-3xl overflow-hidden bg-white text-gray-900 ring-1 ring-inset ring-white/20 lg:ml-auto">
              <div className="p-6 border-b">
                <h2 className="text-xl font-semibold">Instant quote</h2>
                <p className="text-sm text-gray-600 mt-1">Get a ballpark price in seconds.</p>
              </div>
              ...
              <div className="p-6 grid gap-4">
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  {serviceType === "parcel" ? (
                    <>
                      {/* Route */}
                      <LabeledInput label="From (City, Country)" value={parcel.from} onChange={(e)=>setParcel(s=>({...s,from:e.target.value}))} placeholder="Brussels, Belgium" />
                      <LabeledInput label="To (City, Country)"   value={parcel.to}   onChange={(e)=>setParcel(s=>({...s,to:e.target.value}))}     placeholder="London, United Kingdom" />

                      {/* Shipper contact */}
                      <div className="sm:col-span-2 mt-1 text-xs font-semibold text-gray-500">Shipper (Sender)</div>
                      <LabeledInput label="Shipper name"  value={parcel.shipperName}  onChange={(e)=>setParcel(s=>({...s, shipperName: e.target.value}))} placeholder="Your full name / company" />
                      <LabeledInput label="Shipper email" type="email" value={parcel.shipperEmail} onChange={(e)=>setParcel(s=>({...s, shipperEmail: e.target.value}))} placeholder="you@example.com" />
                      <LabeledInput label="Shipper phone" type="tel" value={parcel.shipperPhone} onChange={(e)=>setParcel(s=>({...s, shipperPhone: e.target.value}))} placeholder="+44 801 234 5678" />

                      {/* Recipient contact */}
                      <div className="sm:col-span-2 mt-1 text-xs font-semibold text-gray-500">Recipient</div>
                      <LabeledInput label="Recipient name"  value={parcel.recipientName}  onChange={(e)=>setParcel(s=>({...s, recipientName: e.target.value}))} placeholder="Consignee name" />
                      <LabeledInput label="Recipient phone" type="tel" value={parcel.recipientPhone} onChange={(e)=>setParcel(s=>({...s, recipientPhone: e.target.value}))} placeholder="+44 20 1234 5678" />
                      <LabeledInput label="Recipient email" type="email" value={parcel.recipientEmail} onChange={(e)=>setParcel(s=>({...s, recipientEmail: e.target.value}))} placeholder="receiver@example.com" className="sm:col-span-2" />
                      <LabeledInput label="Recipient address" type="text" value={parcel.recipientAddress} onChange={(e)=>setParcel(s=>({...s, recipientAddress: e.target.value}))} placeholder="123 Example Street, London" className="sm:col-span-2" />
                    </>
                  ) : (
                    <>
                      {/* Route */}
                      <LabeledInput label="From (City, Country)" value={freight.from} onChange={(e)=>setFreight(s=>({...s,from:e.target.value}))} placeholder="Brussels, Nigeria" />
                      <LabeledInput label="To (City, Country)"   value={freight.to}   onChange={(e)=>setFreight(s=>({...s,to:e.target.value}))}     placeholder="Rotterdam, Netherlands" />

                      {/* Shipper contact */}
                      <div className="sm:col-span-2 mt-1 text-xs font-semibold text-gray-500">Shipper (Sender)</div>
                      <LabeledInput label="Shipper name"  value={freight.shipperName}  onChange={(e)=>setFreight(s=>({...s, shipperName: e.target.value}))} placeholder="Your full name / company" />
                      <LabeledInput label="Shipper email" type="email" value={freight.shipperEmail} onChange={(e)=>setFreight(s=>({...s, shipperEmail: e.target.value}))} placeholder="you@example.com" />
                      <LabeledInput label="Shipper phone" type="tel" value={freight.shipperPhone} onChange={(e)=>setFreight(s=>({...s, shipperPhone: e.target.value}))} placeholder="+44 801 234 5678" />

                      {/* Recipient contact */}
                      <div className="sm:col-span-2 mt-1 text-xs font-semibold text-gray-500">Recipient</div>
                      <LabeledInput label="Recipient name"  value={freight.recipientName}  onChange={(e)=>setFreight(s=>({...s, recipientName: e.target.value}))} placeholder="Consignee name" />
                      <LabeledInput label="Recipient phone" type="tel" value={freight.recipientPhone} onChange={(e)=>setFreight(s=>({...s, recipientPhone: e.target.value}))} placeholder="+31 10 123 4567" />
                      <LabeledInput label="Recipient email" type="email" value={freight.recipientEmail} onChange={(e)=>setFreight(s=>({...s, recipientEmail: e.target.value}))} placeholder="consignee@example.com" className="sm:col-span-2" />
                      <LabeledInput label="Recipient address" type="text" value={freight.recipientAddress} onChange={(e)=>setFreight(s=>({...s, recipientAddress: e.target.value}))} placeholder="Warehouse 7, 456 Port Road, Rotterdam" className="sm:col-span-2" />
                    </>
                  )}
                </div>
                ...
                {serviceType === "parcel" ? (
                  <div className="grid sm:grid-cols-2 gap-3">
                    <LabeledInput label="Weight (kg)" value={parcel.weight} onChange={(e)=>setParcel(s=>({...s,weight:e.target.value}))} type="number" min="0" step="0.1" placeholder="2.5" />
                    <div className="grid grid-cols-3 gap-3">
                      <LabeledInput compact label="L (cm)" value={parcel.length} onChange={(e)=>setParcel(s=>({...s,length:e.target.value}))} type="number" min="0" step="1" />
                      <LabeledInput compact label="W (cm)" value={parcel.width}  onChange={(e)=>setParcel(s=>({...s,width:e.target.value}))}  type="number" min="0" step="1" />
                      <LabeledInput compact label="H (cm)" value={parcel.height} onChange={(e)=>setParcel(s=>({...s,height:e.target.value}))} type="number" min="0" step="1" />
                    </div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-3">
                    <div className="grid grid-cols-3 gap-3">
                      <LabeledInput compact label="L (cm)" value={freight.length} onChange={(e)=>setFreight(s=>({...s,length:e.target.value}))} type="number" min="0" step="1" />
                      <LabeledInput compact label="W (cm)" value={freight.width}  onChange={(e)=>setFreight(s=>({...s,width:e.target.value}))}  type="number" min="0" step="1" />
                      <LabeledInput compact label="H (cm)" value={freight.height} onChange={(e)=>setFreight(s=>({...s,height:e.target.value}))} type="number" min="0" step="1" />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <LabeledInput label="Weight (kg / pallet)" value={freight.weight} onChange={(e)=>setFreight(s=>({...s,weight:e.target.value}))} type="number" min="0" step="1" />
                      <LabeledInput label="Pallets" value={freight.pallets} onChange={(e)=>setFreight(s=>({...s,pallets:e.target.value}))} type="number" min="1" step="1" />
                    </div>
                    <div className="sm:col-span-2 grid sm:grid-cols-3 gap-3">
                      <LabeledInput label="Incoterm" value={freight.incoterm} onChange={(e)=>setFreight(s=>({...s,incoterm:e.target.value}))} placeholder="DAP" />
                      <div className="sm:col-span-2 text-xs text-gray-500 self-center">
                        Volumetric (mode-adjusted): <b>{volumetricFreight || 0} kg</b> • Billable est.: <b>{billableFreight || 0} kg</b>
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="rounded-xl border bg-gray-50 p-4">
                  {quote ? (
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                      ...
                      <button
                        ...
                        className={`inline-flex items-center justify-center px-4 py-2 rounded-lg text-sm font-semibold active:scale-[0.98] ${
                          emailOk && addressOk ? "bg-black text-white" : "bg-gray-300 text-gray-600 cursor-not-allowed"
                        }`}
                    ...
                    <div className="text-sm text-gray-600">
                      Fill in the fields to see an estimate.
                    </div>
                  )}
                </div>
              </div>
      ...
      <section className="py-12 sm:py-16 lg:py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl sm:3xl font-bold tracking-tight">How it works</h2>
      ...
      <section className="py-12 sm:py-16 lg:py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="mt-3 text-xs text-gray-500">Times are estimates and may vary by route, weather, or customs requirements. See full terms at checkout.</p>
        </div>
      </section>
      ...
      <section className="py-12 sm:py-16 lg:py-20 bg-white">
        ...
        <div className="mt-6 divide-y rounded-2xl border">
            {[
              { q: "What documents do I need for international shipments?", a: "Commercial invoice, product descriptions with HS codes, and receiver details. Our flow guides you and generates docs automatically." },
              { q: "How is the price calculated?", a: "We charge the greater of actual or volumetric weight plus surcharges. You’ll see a breakdown before you pay." },
      ...
      <section className="py-12 sm:py-16 lg:py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <a href="/#support" className="px-4 py-2 rounded-xl bg-black text-white font-semibold active:scale-[0.98]">Chat with support</a>
              <a href="/#support" className="px-4 py-2 rounded-xl bg-white border font-semibold active:scale-[0.98]">Open a ticket</a>
            </div>
          </div>
...
function LabeledInput({ label, compact, className = "", ...rest }) {
  return (
    <label className="block">
      ...
      <input
        {...rest}
        className={`mt-1 w-full rounded-lg border px-3 py-2 outline-none focus:ring-2 focus:ring-red-500/30 ${compact ? "text-sm" : ""} ${className}`}
      />
    </label>
  );
}
function TogglePill({ label, active, onClick }) {
  return (
    <button type="button" onClick={onClick}
      className={`px-3 py-2 rounded-lg border text-sm font-medium ${active ? "bg-red-600 text-white border-red-600" : "bg-white hover:bg-gray-50"}`}
      aria-pressed={active}>
      {label}
    </button>
  );
...
function Tier({ name, desc, eta, weight, features, highlight, onClick, active }) {
  return (
    <article onClick={onClick}
      ...
      {highlight && <span className="absolute -top-3 right-4 text-[10px] font-semibold px-2 py-1 rounded-full bg-red-600 text-white">Most popular</span>}
      <h3 className="text-lg font-semibold">{name}</h3>
      <p className="mt-1 text-sm text-gray-600">{desc}</p>
      <ul className="mt-4 space-y-2 text-sm text-gray-700">
        <li><CheckIcon /> Transit: {eta}</li>
        <li><CheckIcon /> {weight}</li>
        {features.map((f, i) => <li key={i}><CheckIcon /> {f}</li>)}
      </ul>
    </article>
  );
}
...
/* ---------- tiny icons & utils ---------- */
function LightningIcon() { return <svg viewBox="0 0 24 24" className="w-4 h-4" fill="currentColor"><path d="M11 21 3 13h6L7 3h10l-4 8h6z" /></svg>; }
function CheckIcon() { return <svg viewBox="0 0 24 24" className="inline w-4 h-4 mr-1 -mt-0.5 align-middle" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M5 13l4 4L19 7" strokeLinecap="round" strokeLinejoin="round" /></svg>; }
...
function MapIcon() { return <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.6"><path d="M3 6l6-3 6 3 6-3v15l-6 3-6-3-6 3z" /><path d="M9 3v15M15 6v15" /></svg>; }
function ClockIcon() { return <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.6"><circle cx="12" cy="12" r="9" /><path d="M12 7v6l4 2" /></svg>; }
...
function LockIcon() { return <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.6"><rect x="4" y="10" width="16" height="10" rx="2" /><path d="M8 10V8a4 4 0 0 1 8 0v2" /></svg>; }
function round2(n) { return Math.round(n * 100) / 100; }
function formatMoney(n) { return n.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }); }
function tailCountry(s="") { return s.trim().split(",").slice(-1)[0]?.toLowerCase().trim(); }
...
function FaqItem({ q, a }) {
  ...
  return (
    <div>
      <button onClick={()=>setOpen(!open)} className="w-full flex justify-between items-center py-4 px-4 text-left text-sm font-medium hover:bg-gray-50">
        <span>{q}</span>
        <svg className={`w-4 h-4 transform transition-transform ${open ? "rotate-180" : ""}`} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
